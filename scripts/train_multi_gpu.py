...
def example_convert_to_torch_training(example, dtype=torch.float32,
                             device=None) -> dict:
    device = device or torch.device("cuda:0")
    ...
    for k, v in example[1].items():
        if k in float_names:
            ...
            example_torch2[k] = torch.as_tensor(
                v, dtype=torch.int32, device=device)
        elif k in ["anchors_mask"]:
            example_torch2[k] = torch.as_tensor(
                v, dtype=torch.uint8, device=device)
...
def train(config_path,
    ...
    #net.cuda()
    torchplus.train.try_restore_latest_checkpoints(model_dir, [net])#loadding before doing parallel
    net_paral = multi_gpu_parallel(net, device_ids=[0,1]).cuda()

    #net.volatile=True   ## for wrong access to memory
    ...
    optimizer = optimizer_builder.build(optimizer_cfg, net_paral.module.parameters())
    # optimizer = optimizer_builder.build(optimizer_cfg, net.parameters())
    # mixed_optimizer = optimizer
    # must restore optimizer AFTER using MixedPrecisionWrapper
    torchplus.train.try_restore_latest_checkpoints(model_dir,
                                                   [optimizer]) # loading data before doing parallel
    optimizer = torch.nn.DataParallel(optimizer, device_ids=device_ids) #XXX multi
    lr_scheduler = lr_scheduler_builder.build(optimizer_cfg, optimizer.module, gstep)
