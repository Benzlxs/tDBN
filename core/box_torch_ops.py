...
def tDBN_box_encode(boxes, anchors, encode_angle_to_vector=False, smooth_dim=False):
...
def tDBN_box_decode(box_encodings, anchors, encode_angle_to_vector=False, smooth_dim=False):
...
def multiclass_nms(nms_func,
    ...
    if not (boxes.shape[1] == scores.shape[1] or boxes.shape[1] == 1):
        raise ValueError('tDBN dimension of boxes must be either 1 or equal '
                         'to the tDBN dimension of scores')
