...
def example_convert_to_torch_training_ba(example, dtype=torch.float32,
    ...
    for k, v in example[0].items():
        if k in float_names:
            ...
            example_torch1[k] = tuple(torch.as_tensor(
                v, dtype=torch.int32, device=0))
        elif k in ["anchors_mask"]:
            example_torch1[k] = tuple(torch.as_tensor(
                v, dtype=torch.uint8, device=0))
        else:
    ...
    for k, v in example[1].items():
        if k in float_names:
            ...
            example_torch2[k] = tuple(torch.as_tensor(
                v, dtype=torch.int32, device=1))
        elif k in ["anchors_mask"]:
            example_torch2[k] = tuple(torch.as_tensor(
                v, dtype=torch.uint8, device=1))
        else:
